...
class SceneEnv(rllib.MultiAgentEnv):
    def __init__(self, config):
        ...
        for i in range(self.nrobots):
            ...
            self.target_space = spaces.Dict({
                ...
                "belief": spaces.Box(np.array([0.0]), np.array([distrib.entropy([1/self.number_of_classes]*self.number_of_classes, base=2)+1e-10]), shape=(1,), dtype=np.float32),
                "measurement": spaces.Box(np.array([0.0]), np.array([distrib.entropy([1/self.number_of_classes]*self.number_of_classes, base=2)+1e-10]), shape=(1,), dtype=np.float32),
        ...
        self.env_mode = config["env_mode"] # static / cte_vel: cte velocity / brown: brownian motion / sf_goal: social forces pedestrian dynamics / airsim: airsim simulation
        # self.fix_seed = False
        # self.seed = 1
        self.delta_t = 0.25
        ...
        if self.env_mode == 'sf_goal':
            ...
            self.socialForce_params = {
                'desvel_mean': 1.34,
                'desvel_std': 0.26,
                'potentials': {'target_cte': 2.1,
                               'target_sigma': 0.3, #0.3
                               'wall_cte': 10,
                               'wall_R': 0.6},  #0.2
                'delta_t': 2,
                'tao': 0.5,
    ...
    def load_scenario(self, random_targetID = False):
        ...
        if not random_targetID: self.targets_ID = [int(element) for element in current_scenario[auxid:auxid + self.ntargets]]
        else: self.targets_ID = [random.randint(0, self.number_of_classes - 1) for element in current_scenario[auxid:auxid + self.ntargets]]
        auxid += self.ntargets
    ...
    def init_plot_scene(self, num_figure=1, SIDEx=2*SIDE, SIDEy=2*SIDE):
        self.is_plotting_scene = True

        fig = plt.figure(num_figure)
        ax = fig.gca()
        ax.set_xlim((-SIDEx/2 - 1, SIDEx/2 + 1))
        ax.set_ylim((-SIDEy/2 - 1, SIDEy/2 + 1))
        ax.set_aspect('equal')
        ...
        for t in range(self.ntargets):
            my_item = TargetPlot(shirt_color=self.colors(self.targets_ID[t]),  # np.random.rand(3, ),
                                 ...
                                 target_id=plt.text(self.targets[t][0]+0.5,self.targets[t][1]+0.5,str(t)))
            ...
            ax.add_artist(my_item.target_id)
    ...
    def move_one_target(self, target, delta_t=0.25):
        ...
        max_motion = 1.0*delta_t
        max_rotation = 60.0*delta_t
        if self.dimensions == 3:
            # SCENE3D full motion
            newpos = np.zeros(2)
    ...
    def sf_update(self, dyn_sigma=0):
        """
        ...
        for t1 in range(self.ntargets):
            ...
            candidate_vel = self.targets_vel[t1] + delta_w + np.random.normal(0,dyn_sigma)
    ...
    def new_target_goals(self, t):
        ...
        if goal_dist < MINDIST:
            ...
            new_goal = np.random.uniform(-SIDE,SIDE,2)  # needs to be initialized

            while not valid_new_goal:
                new_goal = np.random.uniform(-SIDE, SIDE, 2)
                valid_new_goal = True
                for t2 in range(self.ntargets):
                    if t==t2: continue
    ...
    def move_targets(self, delta_t = 0.25, dyn_sigma = 0):
        """
        ...
        if self.env_mode =='cte_vel':
            ...
            newPosCand = self.sf_update(dyn_sigma=dyn_sigma)
            self.targets_t1 = np.copy(self.targets)

        for t in range(self.ntargets):
            if self.env_mode != 'static':
                if self.env_mode == 'brown':
                    self.move_one_target(t,delta_t=delta_t)
                if self.env_mode == 'cte_vel':
    ...
    def _obs(self):
        ...
        for r in range(self.nrobots): # compute observation for each robot
            ...
            for t in range(self.ntargets):  # range(self.nrobots):
                ...
                aux["belief"] = np.float32([distrib.entropy([1/self.number_of_classes]*self.number_of_classes, base=2)-distrib.entropy(self.beliefs[0][t, :], base=2)+1e-10])
                distrib.entropy(self.beliefs[0][t, :], base=2)
                aux["measurement"] = np.float32([distrib.entropy([1/self.number_of_classes]*self.number_of_classes, base=2)-distrib.entropy(self._observation[t, r], base=2)+1e-10])
