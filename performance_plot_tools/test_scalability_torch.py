...
if __name__ == '__main__':
    ...
    # Torch policy 1stphase
    addendum = [
        # "REPRODUCIBILITY/our_method/seed0/1stphase",
    ]
    policy_dir += addendum
    ncheckpoint += [6000] * len(addendum)
    experiment_list += [1] * len(addendum)
    ...
    lh_ratio += [1] * len(addendum)
    seeds += [len(addendum)] * int(len(addendum) != 0)
    env_mode_name += ['Ours_Torch_1st'] * int(len(addendum) != 0)
    plot_curve += [True] * int(len(addendum) != 0)

    # Torch policy 1stphase deterministic
    addendum = [
        # "REPRODUCIBILITY/our_method/seed0/1stphase_deterministic",
    ]
    policy_dir += addendum
    ncheckpoint += [6000] * len(addendum)
    experiment_list += [1] * len(addendum)
    ...
    lh_ratio += [1] * len(addendum)
    seeds += [len(addendum)] * int(len(addendum) != 0)
    env_mode_name += ['Ours_Torch_1st_deter'] * int(len(addendum) != 0)
    plot_curve += [True] * int(len(addendum) != 0)

    # Torch policy 2ndphase
    addendum = [
        # "REPRODUCIBILITY/our_method/seed0/2ndphase",
    ]
    policy_dir += addendum
    ncheckpoint += [8000] * len(addendum)
    experiment_list += [1] * len(addendum)
    ...
    lh_ratio += [1] * len(addendum)
    seeds += [len(addendum)] * int(len(addendum) != 0)
    env_mode_name += ['Ours_Torch_2nd'] * int(len(addendum) != 0)
    plot_curve += [True] * int(len(addendum) != 0)

    # Torch policy 1stphase
    addendum = [
        # "REPRODUCIBILITY/our_method_ln/seed0/1stphase",
    ]
    policy_dir += addendum
    ncheckpoint += [5505] * len(addendum)
    experiment_list += [1] * len(addendum)
    heuristic_handcrafted += [False] * len(addendum)
    heuristic_policy += [False] * len(addendum)
    heuristic_target_order += [False] * len(addendum)
    ...
    lh_ratio += [1] * len(addendum)
    seeds += [len(addendum)] * int(len(addendum) != 0)
    env_mode_name += ['Ours_Torch_ln_1st_3503'] * int(len(addendum) != 0)
    plot_curve += [True] * int(len(addendum) != 0)

    # Torch policy_MA 1stphase
    addendum = [
        # "REPRODUCIBILITY/our_method_envMA/seed0/1stphase",
    ]
    policy_dir += addendum
    ncheckpoint += [6000] * len(addendum)
    experiment_list += [1] * len(addendum)
    ...
    lh_ratio += [1] * len(addendum)
    seeds += [len(addendum)] * int(len(addendum) != 0)
    env_mode_name += ['Ours_Torch_1st_MA'] * int(len(addendum) != 0)
    plot_curve += [True] * int(len(addendum) != 0)

    # Torch policy_MA 1stphase SE_Attention_noParamSh
    addendum = [
        "REPRODUCIBILITY/SE_Attention_noParamSh/seed0/1stphase",
    ]
    policy_dir += addendum
    ncheckpoint += [6000] * len(addendum)
    experiment_list += [1] * len(addendum)
    ...
    lh_ratio += [1] * len(addendum)
    seeds += [len(addendum)] * int(len(addendum) != 0)
    env_mode_name += ['Ours_Torch_1st_MA_noParamSh'] * int(len(addendum) != 0)
    plot_curve += [True] * int(len(addendum) != 0)

    # Torch policy_MA 1stphase SE_Attention_noParamSh no done_at_end
    addendum = [
        "REPRODUCIBILITY/SE_Attention_noParamSh_doneatend/seed0/1stphase",
    ]
    policy_dir += addendum
    ncheckpoint += [6000] * len(addendum)
    experiment_list += [1] * len(addendum)
    ...
    lh_ratio += [1] * len(addendum)
    seeds += [len(addendum)] * int(len(addendum) != 0)
    env_mode_name += ['Ours_Torch_1st_MA_noParamSh_daE'] * int(len(addendum) != 0)
    plot_curve += [True] * int(len(addendum) != 0)

    # Torch policy_MA 1stphase SE_Attention_noParamSh done only when task is completed with attention outputs
    addendum = [
        "REPRODUCIBILITY/SE_Attention_noParamSh_doneatend/seed0/1stphase",
    ]
    policy_dir += addendum
    ...
    lh_ratio += [1] * len(addendum)
    seeds += [len(addendum)] * int(len(addendum) != 0)
    env_mode_name += ['Attention_outs'] * int(len(addendum) != 0)
    plot_curve += [True] * int(len(addendum) != 0)

    # Torch policy_MA 1stphase SE_Attention_noParamSh done only when task is completed
    addendum = [
        "REPRODUCIBILITY/SE_Attention_noParamSh_doneatend/seed0/1stphase",
    ]
    policy_dir += addendum
    ncheckpoint += [6000] * len(addendum)
    experiment_list += [3] * len(addendum)
    heuristic_handcrafted += [False] * len(addendum)
    heuristic_policy += [False] * len(addendum)
    heuristic_target_order += [False] * len(addendum)
    robot_target_assignment += [False] * len(addendum)
    ...
    lh_ratio += [1] * len(addendum)
    seeds += [len(addendum)] * int(len(addendum) != 0)
    env_mode_name += ['Ours_Torch_1st_MA_noParamSh_daEtask'] * int(len(addendum) != 0)
    plot_curve += [True] * int(len(addendum) != 0)

    # Torch policy_MA 1stphase SE_Attention_noParamSh done only when task is completed
    addendum = [
        "REPRODUCIBILITY/SE_Attention_noParamSh_doneatend_reviewers/seed0/1stphase",
    ]
    policy_dir += addendum
    ncheckpoint += [1501] * len(addendum)
    experiment_list += [1] * len(addendum)
    heuristic_handcrafted += [False] * len(addendum)
    heuristic_policy += [False] * len(addendum)
    heuristic_target_order += [False] * len(addendum)
    ...
    lh_ratio += [1] * len(addendum)
    seeds += [len(addendum)] * int(len(addendum) != 0)
    env_mode_name += ['1501_rev_trained'] * int(len(addendum) != 0)
    plot_curve += [True] * int(len(addendum) != 0)
