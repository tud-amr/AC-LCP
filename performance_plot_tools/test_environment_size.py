...
def dataextract(episodes, ntargets_list, foldernames, relative_to_heuristic, nrobots_list, side_list):
    info_model = []
...
if __name__ == '__main__':
    ...
    env_mode_name = ['baseline 1 - TA', 'baseline 2 - TA', 'RSS 2022 - 50X50 1stphase', 'RSS 2022 - 50X50 2nd phase']  # +['baseline 2', 'baseline 3 (wip)']
    horizon = 500
    side_list = [8.0,10.0,12.0,17.0,25.0,30,40,50,100]

    plot_curve = [True, True, True, True]
    ...
    extracted_data = dataextract(episodes, ntargets_list, folder_policy_list, relative_to_heuristic, nrobots_list, side_list)
    ...
    for mode in ['success','nsteps','ntracked', 'entropy', 'rewards']:
        ...
        for i,datadict in enumerate(extracted_data):
            if plot_curve[i]:
                ...
                if mode == 'nsteps':
                    ax.plot(datadict['side'], datadict['episode_length_mean'],
                                      label=plotlabel)
                    if stds:
                        ax.fill_between(np.array(datadict['side']), np.array(datadict['episode_length_mean']) - np.array(datadict['episode_length_std']), np.array(datadict['episode_length_mean']) + np.array(datadict['episode_length_std']), alpha =0.5)
                if mode == 'success':
                    ax.plot(datadict['side'], datadict['success'],
                            label=plotlabel)
                if mode == 'ntracked':
                    ax.plot(datadict['side'], datadict['ntracked_mean'],
                                      label=plotlabel)
                    if stds:
                        ...
                        ax.fill_between(np.array(datadict['side']),
                ...
                if mode == 'entropy':
                    ax.plot(datadict['side'], datadict['entropy_mean'],
                            label=plotlabel)
                    if stds:
                        # ax.fill_between(np.array(datadict['ntargets']), np.array(datadict['ntracked_mean']) - np.array(datadict['ntracked_std']), np.array(datadict['ntracked_mean']) + np.array(datadict['ntracked_std']), alpha =0.5)
                        ax.fill_between(np.array(datadict['side']),
                ...
                if mode == 'rewards':
                    ax.plot(datadict['side'], datadict['reward_mean'],
                            label=plotlabel)
                    if stds:
                        # ax.fill_between(np.array(datadict['ntargets']), np.array(datadict['ntracked_mean']) - np.array(datadict['ntracked_std']), np.array(datadict['ntracked_mean']) + np.array(datadict['ntracked_std']), alpha =0.5)
                        ax.fill_between(np.array(datadict['side']),
            ...
            ax.set_xlabel('side of environment')
            if mode == 'success':
