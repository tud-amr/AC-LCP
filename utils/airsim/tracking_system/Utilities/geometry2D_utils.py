...
class Bbox:
    ...
    def translate(self, xxx_todo_changeme, alpha=1):
...
def f_noise(boxA, boxB):
    """
    Check the overlap between :param boxA (Bbox class) and :param boxB (Bbox cass) to decide
     if the small one is noise or not
    :return: small bbox (Bbox cass), Bool
    """
    area_A = f_area(boxA)
    area_B = f_area(boxB)
    ...
    noise = False 
    if intersection >= 0.7*small_area:
        noise = True
    
    return small_box, noise
